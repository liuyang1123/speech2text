...
def main(directory, wavelength=16000, replace=True):
    ...
    if os.path.isdir(directory):
        ...
        if len(mpthree_files > 0):
            ...
            map(lambda x: convert_mp3_to_wav(x, replace=True), mpthree_files.items())

        # now get the wav files after conversion(if any)
        wav_files = find_directory__files(directory, 'wav')

        # convert
        map(lambda x: convert_wavelength_file(x, wavelength=wavelength, replace=True), wav_files.items())
